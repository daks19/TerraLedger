...
function validateEnvironment() {
    ...
    if (missing.length > 0) {
        ...
        missing.forEach(key => logger_1.logger.error(`   - ${key}`));
        logger_1.logger.error('\nCopy .env.example to .env and fill in the values.');
        // Don't process.exit in serverless - throw error instead
        throw new Error(`Missing required environment variables: ${missing.join(', ')}`);
    }
    if (weak.length > 0) {
        logger_1.logger.error('❌ Weak or default environment variables detected in production:');
        weak.forEach(key => logger_1.logger.error(`   - ${key}`));
    ...
    if (warnings.length > 0) {
        logger_1.logger.warn('⚠️  Optional configuration warnings:');
