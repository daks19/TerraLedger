...
exports.resolvers = {
    Query: {
        ...
        user: async (_, { id }, context) => {
            requireRole(context, [client_1.UserRole.ADMIN, client_1.UserRole.GOVERNMENT_OFFICIAL]);
            return context.prisma.user.findUnique({
                where: { id },
                include: { ownedParcels: true },
            });
        },
        userByWallet: async (_, { walletAddress }, context) => {
            return context.prisma.user.findUnique({
        ...
        parcelByParcelId: async (_, { parcelId }, context) => {
            return context.prisma.landParcel.findUnique({
                where: { parcelId },
                include: {
                    owner: true,
        ...
        parcelBySurveyNumber: async (_, { surveyNumber }, context) => {
            return context.prisma.landParcel.findUnique({
                where: { surveyNumber },
                include: { owner: true },
            });
        },
        parcels: async (_, args, context) => {
            const where = {};
            if (args.filter) {
                if (args.filter.district)
                    where.district = args.filter.district;
                if (args.filter.village)
                    where.village = args.filter.village;
                if (args.filter.status)
                    where.status = args.filter.status;
                if (args.filter.isForSale !== undefined)
                    where.isForSale = args.filter.isForSale;
                if (args.filter.minArea || args.filter.maxArea) {
                    where.areaSqM = {};
                    if (args.filter.minArea)
                        where.areaSqM.gte = args.filter.minArea;
                    if (args.filter.maxArea)
                        where.areaSqM.lte = args.filter.maxArea;
                }
            }
        ...
        searchParcels: async (_, { query, limit }, context) => {
        ...
        parcelHistory: async (_, { parcelId }, context) => {
        ...
        transactions: async (_, args, context) => {
            ...
            if (args.filter) {
                if (args.filter.status)
                    where.status = args.filter.status;
                if (args.filter.parcelId)
                    where.parcel = { parcelId: args.filter.parcelId };
            }
        ...
        parcelDisputes: async (_, { parcelId }, context) => {
            return context.prisma.dispute.findMany({
                where: { parcel: { parcelId } },
        ...
        // Inheritance queries
        inheritancePlan: async (_, { id }, context) => {
        ...
        myInheritancePlan: async (_, __, context) => {
            const user = requireAuth(context);
        ...
        inheritancePlansAsHeir: async (_, __, context) => {
            const user = requireAuth(context);
        ...
        dashboardStats: async (_, __, context) => {
            requireRole(context, [client_1.UserRole.ADMIN, client_1.UserRole.GOVERNMENT_OFFICIAL]);
            const [totalParcels, activeParcels, disputedParcels, totalTransactions, pendingTransactions, completedTransactions, totalUsers, verifiedUsers,] = await Promise.all([
                context.prisma.landParcel.count(),
                context.prisma.landParcel.count({ where: { status: client_1.ParcelStatus.ACTIVE } }),
                context.prisma.landParcel.count({ where: { status: client_1.ParcelStatus.DISPUTED } }),
                context.prisma.transaction.count(),
                context.prisma.transaction.count({ where: { status: client_1.TransactionStatus.PENDING } }),
                context.prisma.transaction.count({ where: { status: client_1.TransactionStatus.COMPLETED } }),
                context.prisma.user.count(),
                context.prisma.user.count({ where: { kycStatus: client_1.KYCStatus.VERIFIED } }),
            ]);
            return {
                totalParcels,
                activeParcels,
    ...
    Mutation: {
        ...
        register: async (_, { input }, context) => {
            ...
            if (existingUser) {
                throw new graphql_1.GraphQLError('Wallet address already registered. This MetaMask wallet is already associated with another account. Each wallet can only be used for one TerraLedger account.', {
                    extensions: { code: 'BAD_USER_INPUT' },
                });
            }
            // Verify wallet signature (skipped in development mode)
            if (!(0, signature_1.shouldSkipSignatureVerification)()) {
                const isValid = await (0, signature_1.verifyWalletSignature)(input.walletAddress, input.signature);
                if (!isValid) {
                    throw new graphql_1.GraphQLError('Invalid signature. Please sign the message with your wallet.', {
        ...
        login: async (_, { walletAddress, signature }, context) => {
            ...
            if (!user.isActive) {
                throw new graphql_1.GraphQLError('Account deactivated', {
                    extensions: { code: 'FORBIDDEN' },
                });
            }
            // Verify wallet signature (skipped in development mode)
            if (!(0, signature_1.shouldSkipSignatureVerification)()) {
                const isValid = await (0, signature_1.verifyWalletSignature)(walletAddress, signature);
                if (!isValid) {
                    throw new graphql_1.GraphQLError('Invalid signature. Please sign the message with your wallet.', {
                        extensions: { code: 'UNAUTHORIZED' },
                    });
        ...
        verify2FA: async (_, { code }, context) => {
            const user = requireAuth(context);
            const dbUser = await context.prisma.user.findUnique({
                where: { id: user.id },
            });
            if (!dbUser?.twoFactorSecret) {
                throw new graphql_1.GraphQLError('2FA not set up', {
            ...
            if (!isValid) {
                throw new graphql_1.GraphQLError('Invalid 2FA code', {
                    extensions: { code: 'BAD_REQUEST' },
                });
            }
            await context.prisma.user.update({
        ...
        // Parcel mutations
        registerParcel: async (_, { input }, context) => {
            const user = requireRole(context, [client_1.UserRole.GOVERNMENT_OFFICIAL, client_1.UserRole.ADMIN]);
            // Generate unique parcel ID
            const parcelId = `${input.state.substring(0, 2).toUpperCase()}-${input.district.substring(0, 2).toUpperCase()}-${Date.now()}`;
            // Register on blockchain
            const txHash = await context.blockchainService.registerParcel(parcelId, user.walletAddress, input.areaSqM, input.surveyNumber, input.village, input.district, input.state, input.boundaryGeoJSON, input.documentsHash || '');
        ...
        listParcelForSale: async (_, { parcelId, price }, context) => {
            const user = requireAuth(context);
            const parcel = await context.prisma.landParcel.findUnique({
                where: { parcelId },
            });
            if (!parcel || parcel.ownerId !== user.id) {
                throw new graphql_1.GraphQLError('Parcel not found or not owned', {
                    extensions: { code: 'NOT_FOUND' },
                });
            }
            if (parcel.status === client_1.ParcelStatus.DISPUTED) {
                throw new graphql_1.GraphQLError('Cannot list disputed parcel', {
                    extensions: { code: 'BAD_REQUEST' },
                });
            }
            // List on blockchain
        ...
        createTransaction: async (_, { input }, context) => {
            ...
            if (!parcel) {
                throw new graphql_1.GraphQLError('Parcel not found', {
                    extensions: { code: 'NOT_FOUND' },
                });
            }
            if (!parcel.isForSale) {
                throw new graphql_1.GraphQLError('Parcel not for sale', {
                    extensions: { code: 'BAD_REQUEST' },
                });
            }
            // Create escrow on blockchain
            const escrowId = await context.blockchainService.createEscrow(input.parcelId, parcel.owner.walletAddress, input.amount);
        ...
        approveTransaction: async (_, { transactionId }, context) => {
            const user = requireAuth(context);
            ...
            if (!transaction) {
                throw new graphql_1.GraphQLError('Transaction not found', {
                    extensions: { code: 'NOT_FOUND' },
                });
            }
            let updateData = {};
            ...
            else if (user.roles.includes(client_1.UserRole.GOVERNMENT_OFFICIAL)) {
                await context.blockchainService.governmentApprove(transaction.escrowId);
                updateData.govtApproved = true;
                updateData.status = client_1.TransactionStatus.VERIFIED;
            }
            else {
                throw new graphql_1.GraphQLError('Not authorized to approve', {
                    extensions: { code: 'FORBIDDEN' },
                });
            }
        ...
        createInheritancePlan: async (_, { input }, context) => {
            const user = requireAuth(context);
            ...
            if (parcels.length !== input.parcelIds.length) {
                throw new graphql_1.GraphQLError('You do not own all specified parcels', {
                    extensions: { code: 'BAD_REQUEST' },
                });
            }
            // Create plan on blockchain
            const planId = await context.blockchainService.createInheritancePlan(input.parcelIds, input.useAgeMilestones || false);
        ...
        addHeir: async (_, { input }, context) => {
            ...
            if (!plan || plan.ownerId !== user.id) {
                throw new graphql_1.GraphQLError('Plan not found or not owned', {
                    extensions: { code: 'NOT_FOUND' },
                });
            }
            ...
            if (currentTotal + input.percentage > 100) {
                throw new graphql_1.GraphQLError('Total percentage exceeds 100%', {
                    extensions: { code: 'BAD_REQUEST' },
                });
            }
            ...
            if (!heirUser) {
                try {
                    ...
                    if (error.code === 'P2002') {
                        throw new graphql_1.GraphQLError('Wallet address already registered. This wallet is already associated with another account.', {
                            extensions: { code: 'BAD_USER_INPUT' },
                        });
                    }
                    throw error;
            ...
            await context.blockchainService.addHeir(plan.planId, input.walletAddress, input.percentage, input.releaseAge || 0, input.birthDate ? new Date(input.birthDate).getTime() / 1000 : 0);
        ...
        triggerInheritance: async (_, { planId, deathCertHash }, context) => {
            requireRole(context, [client_1.UserRole.GOVERNMENT_OFFICIAL, client_1.UserRole.ADMIN]);
            const plan = await context.prisma.inheritancePlan.findUnique({
                where: { id: planId },
            });
            if (!plan) {
                throw new graphql_1.GraphQLError('Plan not found', {
                    extensions: { code: 'NOT_FOUND' },
                });
            }
        ...
        // Dispute mutations
        fileDispute: async (_, { input }, context) => {
            const user = requireAuth(context);
            const parcel = await context.prisma.landParcel.findUnique({
                where: { parcelId: input.parcelId },
            });
            ...
            const disputeId = await context.blockchainService.fileDispute(input.parcelId, input.affectedParcelIds || [], input.description, input.evidenceHash || '');
            // Update parcel status
            await context.prisma.landParcel.update({
                where: { id: parcel.id },
                data: { status: client_1.ParcelStatus.DISPUTED },
            });
        ...
        resolveDispute: async (_, { disputeId, resolution, details, newBoundaryHash }, context) => {
            requireRole(context, [client_1.UserRole.GOVERNMENT_OFFICIAL, client_1.UserRole.ADMIN]);
            ...
            if (!dispute) {
                throw new graphql_1.GraphQLError('Dispute not found', {
                    extensions: { code: 'NOT_FOUND' },
                });
            }
            // Resolve on blockchain
            await context.blockchainService.resolveDispute(dispute.disputeId, resolution, details, newBoundaryHash || '');
            // Update parcel status
            await context.prisma.landParcel.update({
                where: { id: dispute.parcelId },
                data: { status: client_1.ParcelStatus.ACTIVE },
            });
    ...
    // Field resolvers
    User: {
        ownedParcels: async (parent, _, context) => {
            return context.prisma.landParcel.findMany({
