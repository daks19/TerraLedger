...
router.get('/records/search', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Public search error:', error);
...
router.get('/records/:parcelId', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Public record fetch error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
/**
    ...
    try {
        ...
        try {
            ...
            if (!onChainParcel) {
                ...
                const getField = (name, idx) => {
                    const v = onChainParcel?.[name];
                    if (v !== undefined)
                        return v;
                    return onChainParcel?.[idx];
                };
                ...
                if (hashToCheck) {
                    ...
                    const checks = [boundaryMatch, docMatch].filter(v => v !== null);
 ...
 */
router.get('/map/parcels', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Map parcels error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
/**
