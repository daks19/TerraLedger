...
router.get('/boundaries/:parcelId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Get boundaries error:', error);
...
router.post('/validate', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Validate boundaries error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

// GET /api/map/parcels-in-bounds
router.get('/parcels-in-bounds', async (req: Request, res: Response) => {
  try {
    ...
    const parcelsWithBoundaries = await Promise.all(
    ...
    logger.error('Get parcels in bounds error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.get('/search', async (req: Request, res: Response) => {
  try {
    ...
    const radiusKm = parseFloat(radius as string) || 1; // Default 1km radius
...
router.post('/calculate-area', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Calculate area error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
function validateGeoJSON(geoJSON: any): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
...
async function checkOverlaps(parcelId: string, boundary: any): Promise<string[]> {
...
function calculateSphericalArea(geoJSON: any): number {
